...
class DemandesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create demande" do
    assert_difference("Demande.count") do
      post demandes_url, params: { demande: { icon: @demande.icon, image: @demande.image, name: @demande.name } }
    end
  ...
  test "should update demande" do
    patch demande_url(@demande), params: { demande: { icon: @demande.icon, image: @demande.image, name: @demande.name } }
